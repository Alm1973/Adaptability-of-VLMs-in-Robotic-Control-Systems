...
- Benchmarks like SpatialBotBench and EmbSpatial show large frontier models (o3, Gemini-2.5-Pro) scoring above 90% on simple position relationships (left/right/front/back) and proximity (near/far), which the OmniSpatial benchmark authors describe as an approaching-saturation task for that model tier.
